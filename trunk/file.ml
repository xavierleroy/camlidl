...
let process_import oc = function
    Comp_constdecl cd -> Constdecl.import cd
  | c -> declare_comp oc c
...
let gen_c_stub oc imports intf =
  ...
  (* Include the .h for the module (as generated by e.g. MIDL) *)
  if !Clflags.include_header then
    fprintf oc "\n#include \"%s.h\"\n\n" !module_name;
  (* Declare the conversion functions for the imports *)
  List.iter (process_import oc) imports;
