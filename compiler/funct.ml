...
let remove_dependent_parameters params =
  let rec remove = function
    [] -> []
  | (name, _, _ as param) :: rem ->
      if is_dependent_parameter name params
      then remove rem
      else param :: remove rem
  in remove params
...
let declare_ml_function oc fundecl =
  let (ins, outs) = ml_view fundecl in
  fprintf oc "external %s : " fundecl.fun_name;
  out_ml_types oc "->" ins;
  fprintf oc " -> ";
...
let function_wrapper oc fundecl =
  ...
  List.iter
    (function (name, _, Type_pointer(attr, ty_arg))
